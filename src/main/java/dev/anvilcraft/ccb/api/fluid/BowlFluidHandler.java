package dev.anvilcraft.ccb.api.fluid;
...
public class BowlFluidHandler implements IFluidHandlerItem {
    ...
    public FluidStack getFluidInTank(int i) {
        if (container.is(CCBItems.MUSH_BAR_BOWL)) {
            return new FluidStack(CCBFluids.GENETIC_OOZE, BOWL_PER_BOTTLE);
        }
    ...
    private static ItemStack getFilledBowl(FluidStack resource) {
        if (resource.getFluid() instanceof GeneticOoze) {
            return new ItemStack(CCBItems.MUSH_BAR_BOWL.asItem());
        }
