package dev.anvilcraft.ccb.client.renderer.item;
...
public class WormItemRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    public void renderByItem(
        ...
        if (!stack.is(CCBBlocks.CEMENT_WORM_BLOCK.asItem())) return;

        ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();
        // 物品模型是 builtin/entity，自身没有几何体，方块模型由渲染器自行绘制
        BakedModel model = Minecraft.getInstance()
            .getBlockRenderer()
            .getBlockModel(CCBBlocks.CEMENT_WORM_BLOCK.get().defaultBlockState());
